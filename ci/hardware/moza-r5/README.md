...
If a real response receipt exists but does not satisfy the visible-motion threshold, do not rerun, raise force, or replace `native-actuator-visible-smoke.json` from generated guidance. Create a non-claiming follow-up plan first, preserve the response receipt, inspect bench/setup/FFB mode, and require a fresh bench-clear for the exact next output command:
