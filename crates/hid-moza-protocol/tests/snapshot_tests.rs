...
use racing_wheel_hid_moza_protocol::{
    ...
    MozaProtocol, REPORT_LEN, es_compatibility, identify_device,
...
fn test_snapshot_is_wheelbase_known_wheelbases() {
    ...
    let results: Vec<(u16, bool)> =
        wheelbases.iter().map(|&pid| (pid, is_wheelbase_product(pid))).collect();
...
fn test_snapshot_is_wheelbase_peripherals() {
    ...
    let results: Vec<(u16, bool)> =
        peripherals.iter().map(|&pid| (pid, is_wheelbase_product(pid))).collect();
    assert_snapshot!(format!("{results:?}"));
}
