...
pub async fn execute(cmd: &SafetyCommands, json: bool, endpoint: Option<&str>) -> Result<()> {
    ...
    match cmd {
        ...
        SafetyCommands::Limit { device, torque, global } => {
...
async fn enable_high_torque(
    ...
    if !can_enable && !force {
        ...
        if json {
            ...
            println!("\n{} Use --force to override safety checks", "Warning:".yellow().bold());
        }
    ...
    if !force && !json {
        ...
        println!("This will enable high torque output up to {:.1} Nm", status.device.capabilities.max_torque_nm);
        println!("Ensure:");
...
async fn emergency_stop(client: &WheelClient, device: Option<&str>, json: bool) -> Result<()> {
    match device {
        Some(device_id) => {
            // Verify device exists
            let _status = client.get_device_status(device_id).await
        ...
        None => {
            ...
            output::print_success(
                "Emergency stop executed for all devices",
                json
            );
        }
    }
...
async fn show_safety_status(
    ...
    if let Some(device_id) = device {
        ...
        let status = client.get_device_status(device_id).await
            .map_err(|_| CliError::DeviceNotFound(device_id.to_string()))?;
...
async fn set_torque_limit(
    ...
    if torque > max_torque {
        return Err(CliError::ValidationError(
            format!("Torque limit {:.1} Nm exceeds device maximum {:.1} Nm", torque, max_torque)
        ).into());
    }
    
    if torque < 0.1 {
        return Err(CliError::ValidationError(
            "Torque limit must be at least 0.1 Nm".to_string()
        ).into());
    }
    
    // Mock torque limit setting - in real implementation this would update device/profile
    
    let scope = if global { "all profiles" } else { "current session" };
    
    output::print_success(
        &format!("Torque limit set to {:.1} Nm for device {} ({})", torque, device, scope),
        json
    );
    
    if !json {
        if torque < max_torque * 0.5 {
            println!("{} Low torque limit may reduce force feedback quality", "Note:".yellow());
        }
...
fn print_device_safety_status(status: &DeviceStatus, safety: &SafetyStatus) {
    ...
    let hands_status = if safety.hands_on { "✓".green() } else { "✗".red() };
