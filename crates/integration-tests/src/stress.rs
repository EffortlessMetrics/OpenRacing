...
use crate::{StressLevel, TestConfig, TestResult};
...
pub async fn test_hotplug_stress() -> Result<TestResult> {
            ...
            match disconnect_result {
                Ok(Ok(_)) => {
                    ...
                    if reconnect_verification.is_err() {
                        failed_cycles += 1;
                        errors.push(format!(
                            "Device {} reconnection failed in cycle {}",
    ...
    if success_rate < 95.0 {
        errors.push(format!(
            "Hot-plug success rate {:.2}% below 95% threshold",
            success_rate
        ));
    }
...
pub async fn test_fault_injection_stress() -> Result<TestResult> {
    ...
    while Instant::now() < end_time {
        for &fault_type in &fault_types {
            ...
            if fault_response_time > Duration::from_millis(50) {
                errors.push(format!(
                    "Fault {} response time {:?} exceeded 50ms",
    ...
    let actual_duration = start_time.elapsed();
    let recovery_rate =
        ((fault_injection_count - recovery_failures) as f64 / fault_injection_count as f64) * 100.0;
...
pub async fn test_memory_pressure_stress() -> Result<TestResult> {
    ...
    let _pressure_tasks: Vec<_> = (0..4)
        .map(|i| tokio::spawn(async move { simulate_memory_pressure(i).await }))
        .collect();
    ...
    while Instant::now() < end_time {
        ...
        if current_metrics.memory_usage_mb > 150.0 {
            errors.push(format!(
                "Memory usage {:.1}MB exceeded 150MB limit",
        ...
        if current_metrics.jitter_p99_ms > crate::MAX_JITTER_P99_MS {
            errors.push(format!(
                "Jitter {:.3}ms exceeded limit under memory pressure",
...
pub async fn test_cpu_load_stress() -> Result<TestResult> {
    ...
    let _load_tasks: Vec<_> = (0..num_cpus::get() - 1)
        .map(|i| tokio::spawn(async move { simulate_cpu_load(i).await }))
        .collect();
    ...
    while Instant::now() < end_time {
        ...
        if current_metrics.cpu_usage_percent > 3.0 {
            warn!(
                "Service CPU usage {:.1}% exceeded 3% limit",
        ...
        if current_metrics.jitter_p99_ms > crate::MAX_JITTER_P99_MS {
            errors.push(format!(
                "Jitter {:.3}ms exceeded limit under CPU load",
                current_metrics.jitter_p99_ms
            ));
        }
