...
use crate::{
    FFB_FREQUENCY_HZ, MAX_HID_LATENCY_P99_US, MAX_JITTER_P99_MS, PerformanceMetrics, TestConfig,
    TestResult,
};
...
pub async fn test_ffb_jitter_gate() -> Result<TestResult> {
    ...
    info!(
        "Jitter test completed: {} ticks in {:?}",
...
pub async fn test_hid_latency_gate() -> Result<TestResult> {
    ...
    info!(
        "Starting HID write latency measurement for {:?}",
        test_duration
    );
    ...
    let actual_duration = start_time.elapsed();
    info!(
        "HID latency test completed: {} writes in {:?}",
        total_writes, actual_duration
    );
...
pub async fn test_zero_missed_ticks_gate() -> Result<TestResult> {
    ...
    let actual_duration = start_time.elapsed();
    info!(
        "Zero missed ticks test completed: {} ticks in {:?}",
        total_ticks, actual_duration
    );
    info!(
        "Missed ticks: {} / {} ({:.6}%)",
...
pub async fn test_combined_load_gate() -> Result<TestResult> {
    ...
    let actual_duration = start_time.elapsed();
    info!(
        "Combined load test completed: {} ticks in {:?}",
        total_ticks, actual_duration
    );
