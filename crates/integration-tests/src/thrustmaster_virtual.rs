//! Virtual Thrustmaster device for integration and e2e testing.
//!
//! `VirtualThrustmasterDevice` implements `DeviceWriter` so protocol code can be
//! tested without real hardware. It records feature reports and output reports in
...
/// A software stand-in for a Thrustmaster HID device used in integration tests.
///
/// Records all feature reports and output reports written to it so tests
/// can assert on the exact bytes sent.
pub struct VirtualThrustmasterDevice {
    pub product_id: u16,
    pub vendor_id: u16,
...
impl VirtualThrustmasterDevice {
...
impl DeviceWriter for VirtualThrustmasterDevice {
    fn write_feature_report(&mut self, data: &[u8]) -> Result<usize, Box<dyn std::error::Error>> {
        if self.fail_writes {
            return Err("VirtualThrustmasterDevice: simulated write failure".into());
        }
        let len = data.len();
...
pub struct ThrustmasterScenario {
    pub protocol: ThrustmasterProtocolHandler,
    pub device: VirtualThrustmasterDevice,
}

impl ThrustmasterScenario {
    /// Create a scenario for a known wheelbase product.
    pub fn wheelbase(product_id: u16) -> Self {
        Self {
            protocol: ThrustmasterProtocolHandler::new(THRUSTMASTER_VENDOR_ID, product_id),
            device: VirtualThrustmasterDevice::new(product_id),
        }
    }
    ...
    pub fn wheelbase_failing(product_id: u16) -> Self {
        Self {
            protocol: ThrustmasterProtocolHandler::new(THRUSTMASTER_VENDOR_ID, product_id),
            device: VirtualThrustmasterDevice::new_failing(product_id),
        }
    }
