...
impl VirtualCammusDevice {
    ...
    pub fn model(&self) -> CammusModel {
        CammusModel::from_product_id(self.product_id)
    }
}
...
impl DeviceWriter for VirtualCammusDevice {
    fn write_feature_report(&mut self, data: &[u8]) -> Result<usize, Box<dyn std::error::Error>> {
        if self.fail_writes {
            return Err("VirtualCammusDevice: simulated write failure".into());
        }
        let len = data.len();
...
impl CammusScenario {
    pub fn wheelbase(product_id: u16) -> Self {
        Self {
            protocol: CammusProtocolHandler::new(CAMMUS_VENDOR_ID, product_id),
            device: VirtualCammusDevice::new(product_id),
        }
    }
    ...
    pub fn wheelbase_failing(product_id: u16) -> Self {
        Self {
            protocol: CammusProtocolHandler::new(CAMMUS_VENDOR_ID, product_id),
            device: VirtualCammusDevice::new_failing(product_id),
        }
    }
