...
use racing_wheel_engine::{
    ExpectedResponse, RTLoopTestHarness, TestHarnessConfig, TestScenario, TorquePattern,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Max update rate: {:.0} Hz",
        capabilities.max_update_rate_hz()
    );
    println!(
        "  Supports raw torque: {}",
        capabilities.supports_raw_torque_1khz
    );
    ...
    println!("\n✅ Requirements Validation:");
    println!(
        "  DM-01 (Enumeration < 300ms): {}",
        if enumeration_time < Duration::from_millis(300) {
            "PASS"
        } else {
            "FAIL"
        }
    ...
    println!(
        "  NFR-01 (P99 jitter ≤ 250μs): {}",
        if jitter_requirement { "PASS" } else { "FAIL" }
    );
    println!(
        "  NFR-01 (Missed ticks ≤ 0.001%): {}",
        if missed_tick_requirement {
            "PASS"
        } else {
            "FAIL"
        }
