...
fn create_test_telemetry(rpm: f32, speed_ms: f32, slip_ratio: f32, gear: i8) -> NormalizedTelemetry {
    NormalizedTelemetry {
        ffb_scalar: 0.5,
        rpm,
        speed_ms,
...
mod led_mapping_engine_tests {
    ...
    fn test_rpm_hysteresis() {
        ...
        assert!(lit_count3 > lit_count1, "RPM increase should light more LEDs");
    }
    ...
    async fn test_pattern_timing() {
        ...
        assert!(has_on && has_off, "Blink pattern should alternate between on and off");
    }
...
mod haptics_router_tests {
    ...
    fn test_slip_based_haptics() {
        ...
        if let Some(HapticsPattern::RimVibration { intensity, frequency_hz }) = patterns.get("rim_slip") {
    ...
    fn test_engine_vibration() {
        ...
        if let Some(HapticsPattern::EngineVibration { intensity, rpm_multiplier, .. }) = patterns.get("engine") {
            assert!(*intensity > 0.0);
    ...
    fn test_abs_haptics() {
        ...
        if let Some(HapticsPattern::PedalFeedback { pedal, intensity, .. }) = patterns.get("brake_abs") {
...
mod dash_widget_tests {
    ...
    fn test_rpm_widget() {
        ...
        if let Some(DashWidget::Rpm { current_rpm, max_rpm, redline_rpm }) = widgets.get("rpm") {
...
mod rate_independence_tests {
    ...
    async fn test_rate_independence_200hz() {
        ...
        let output_handle = tokio::spawn(async move {
            ...
            while start_time.elapsed() < Duration::from_millis(500) { // 0.5 second window
...
mod performance_tests {
    ...
    fn test_led_pattern_generation_performance() {
        ...
        assert!(per_update < 1_000_000, "Per-update time too high: {} ns", per_update);
    }
