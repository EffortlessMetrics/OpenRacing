...
async fn test_device_enumeration_and_refresh() {
    ...
    // Refresh devices
    port.refresh_devices().await.expect("Failed to refresh devices");
    
    // Second enumeration should work
    let devices2 = port.list_devices().await.expect("Failed to list devices after refresh");
...
async fn test_rt_safe_ffb_writing() {
    ...
    assert!(error_rate < 0.1, "Error rate too high: {:.2}%", error_rate * 100.0);
}
...
async fn test_telemetry_reading_consistency() {
    ...
    for (i, telemetry) in telemetry_readings.iter().enumerate() {
        ...
        assert!(
            ...
            "Reading {}: Invalid fault flags: 0x{:02X}", i, telemetry.fault_flags
        );
    }
}
...
async fn test_device_capabilities_validation() {
    ...
    for device_info in &devices {
        ...
        if caps.supports_raw_torque_1khz {
            assert!(caps.min_report_period_us <= 1000, 
                "Device claims 1kHz support but min period is {}μs", caps.min_report_period_us);
        }
        
        // Validate torque range
        assert!(caps.max_torque.value() > 0.0 && caps.max_torque.value() <= 50.0,
            "Invalid max torque: {} Nm", caps.max_torque.value());
        
        // Validate encoder resolution
        assert!(caps.encoder_cpr >= 100 && caps.encoder_cpr <= 65535,
            "Invalid encoder CPR: {}", caps.encoder_cpr);
        
        // Validate minimum report period
        assert!(caps.min_report_period_us >= 100 && caps.min_report_period_us <= 65535,
            "Invalid min report period: {}μs", caps.min_report_period_us);
        
        // At least one FFB mode should be supported
        assert!(caps.supports_pid || caps.supports_raw_torque_1khz,
...
async fn test_error_handling_and_recovery() {
    ...
    let devices = port.list_devices().await.expect("List devices should work after error");
    println!("Device enumeration recovered, found {} devices", devices.len());
...
async fn test_ffb_write_latency_benchmark() {
    ...
    assert!(avg < Duration::from_micros(500), "Average latency too high: {:?}", avg);
}