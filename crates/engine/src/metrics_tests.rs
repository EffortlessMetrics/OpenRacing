...
mod metrics_validation_tests {
    ...
    fn test_rt_performance_validation_nfr01() {
        ...
        assert!(violations.is_empty(), "Good metrics should not have violations: {:?}", violations);
    ...
    fn test_system_resource_validation_nfr02() {
        ...
        let base_metrics = RTMetrics {
            total_ticks: 1000,
            missed_ticks: 0,
            jitter_ns: JitterStats { p50_ns: 100_000, p99_ns: 200_000, max_ns: 240_000 },
            hid_latency_us: LatencyStats { p50_us: 150, p99_us: 250, max_us: 290 },
            processing_time_us: LatencyStats { p50_us: 40, p99_us: 180, max_us: 195 },
    ...
    fn test_missed_tick_rate_validation() {
        ...
        let calculate_missed_rate = |missed: u64, total: u64| -> f64 {
            if total == 0 { 0.0 } else { (missed as f64 / total as f64) * 100.0 }
        };
        
        assert_eq!(calculate_missed_rate(missed_ticks_acceptable, total_ticks), 0.0);
        assert_eq!(calculate_missed_rate(missed_ticks_warning, total_ticks), 0.1);
        assert_eq!(calculate_missed_rate(missed_ticks_critical, total_ticks), 0.5);
    ...
    fn test_prometheus_metrics_export() {
        ...
        let rt_metrics = RTMetrics {
            total_ticks: 5000,
            missed_ticks: 2,
            jitter_ns: JitterStats { p50_ns: 100_000, p99_ns: 200_000, max_ns: 300_000 },
            hid_latency_us: LatencyStats { p50_us: 150, p99_us: 250, max_us: 350 },
            processing_time_us: LatencyStats { p50_us: 50, p99_us: 180, max_us: 220 },
    ...
    async fn test_system_monitor_accuracy() {
        ...
        let mem_diff = if mem2 > mem1 { mem2 - mem1 } else { mem1 - mem2 };
        assert!(mem_diff < 10 * 1024 * 1024, "Memory usage changed by more than 10MB: {} -> {}", mem1, mem2);
...
mod metrics_performance_tests {
    ...
    async fn benchmark_prometheus_metrics_performance() {
        ...
        println!("Prometheus update performance: {:.0} updates/sec", updates_per_sec);
        
        // Should be able to handle at least 1000 updates/sec (much higher than collection rate)
        assert!(updates_per_sec > 1000.0, "Prometheus updates too slow: {:.0} updates/sec", updates_per_sec);
    }
}