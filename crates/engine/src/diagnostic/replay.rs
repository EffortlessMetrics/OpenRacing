...
use super::{
    blackbox::{IndexEntry, WbbFooter, WbbHeader},
    streams::{StreamARecord, StreamBRecord, StreamCRecord, StreamReader},
};
use crate::{pipeline::Pipeline, rt::Frame, safety::SafetyService};
...
impl BlackboxReplay {
    ...
    pub fn load_from_file(file_path: &Path, config: ReplayConfig) -> Result<Self, String> {
        let mut file =
            File::open(file_path).map_err(|e| format!("Failed to open blackbox file: {}", e))?;
    ...
    fn read_header(file: &mut File) -> Result<WbbHeader, String> {
        codec::decode_from_std_read(file).map_err(|e| format!("Failed to decode header: {}", e))
    }
    ...
    fn read_footer(file: &mut File) -> Result<WbbFooter, String> {
        ...
        let footer: WbbFooter = codec::decode_from_slice(&footer_bytes)
