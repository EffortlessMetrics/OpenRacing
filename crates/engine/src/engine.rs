...
use crate::{
    rt::{Frame, RTResult, RTError, PerformanceMetrics},
...
impl Engine {
    ...
    pub async fn start(&mut self, device: Box<dyn HidDevice>) -> Result<(), String> {
        ...
        // Apply RT setup
        rt_context.scheduler.apply_rt_setup(&self.config.rt_setup)
        ...
        let rt_thread = thread::Builder::new()
            ...
            .spawn(move || {
                Self::rt_thread_main(rt_context)
            })
    ...
    pub async fn apply_pipeline(&self, pipeline: CompiledPipeline) -> Result<(), String> {
        if let Some(ref command_tx) = self.command_tx {
            ...
            command_tx.try_send(EngineCommand::ApplyPipeline {
                pipeline,
    ...
    pub fn update_safety(&self, hands_on: bool, device_temp_c: u8) -> Result<(), String> {
        if let Some(ref command_tx) = self.command_tx {
            command_tx.try_send(EngineCommand::UpdateSafety {
                hands_on,
                device_temp_c,
    ...
    pub async fn get_stats(&self) -> Result<EngineStats, String> {
        if let Some(ref command_tx) = self.command_tx {
            ...
            command_tx.try_send(EngineCommand::GetStats {
                response: response_tx,
    ...
    fn rt_thread_main(mut ctx: RTContext) -> RTResult {
        ...
        while ctx.running.load(Ordering::Acquire) {
            ...
            let mut frame = Frame {
                ...
                torque_out: 0.0,
                wheel_speed: last_game_input.telemetry
                    .as_ref()
    ...
    fn process_commands(ctx: &mut RTContext) -> RTResult {
        ...
        while let Ok(command) = ctx.command_rx.try_recv() {
            match command {
                ...
                EngineCommand::UpdateSafety { hands_on, device_temp_c } => {
