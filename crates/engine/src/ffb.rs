...
impl CapabilityNegotiator {
    ...
    pub fn parse_capabilities_report(report: &[u8]) -> Result<DeviceCapabilities, String> {
        ...
        if report[0] != 0x01 {
            return Err(format!("Invalid report ID: expected 0x01, got 0x{:02x}", report[0]));
        }
        ...
        let max_torque = TorqueNm::from_cnm(max_torque_cnm)
            .map_err(|e| format!("Invalid max torque: {}", e))?;
    ...
    pub fn create_capabilities_report(caps: &DeviceCapabilities) -> Vec<u8> {
        ...
        let mut flags = 0u8;
        if caps.supports_pid { flags |= 0x01; }
        if caps.supports_raw_torque_1khz { flags |= 0x02; }
        if caps.supports_health_stream { flags |= 0x04; }
        if caps.supports_led_bus { flags |= 0x08; }
        report[1] = flags;
