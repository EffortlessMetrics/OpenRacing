...
impl ProfileService {
    ...
    pub async fn initialize(&self) -> Result<(), ProfileServiceError> {
        ...
        let global_profile = match self.repo.load_global_profile().await {
            ...
            Err(ProfileRepoError::ProfileNotFound(_)) => {
                ...
                let default_profile = Profile::default_global()
                    .map_err(ProfileServiceError::DomainError)?;
                
                self.repo.save_global_profile(&default_profile).await
    ...
    pub async fn resolve_profile(
        ...
        let game_profile = if let Some(ref game) = context.game {
            match self.load_profile_for_scope(&ProfileScope::for_game(game.clone())).await {
    ...
    pub async fn load_profile(&self, id: &ProfileId) -> Result<Profile, ProfileServiceError> {
        ...
        let profile = self.repo.load_profile(id).await
            .map_err(ProfileServiceError::RepoError)?;
    ...
    pub async fn save_profile(
        ...
        profile.validate_for_device(device_capabilities)
            .map_err(ProfileServiceError::DomainError)?;
    ...
    pub async fn validate_profile(
        ...
        if let Err(e) = profile.validate_for_device(device_capabilities) {
            ...
            result.errors.push(format!("Device capability validation failed: {}", e));
        }
        ...
        if profile.base_settings.ffb_gain.value() > 0.9 {
            result.warnings.push("FFB gain is very high (>90%), consider reducing for safety".to_string());
        }
        
        if profile.base_settings.torque_cap.value() > 20.0 {
            result.warnings.push("Torque cap is very high (>20Nm), ensure proper safety measures".to_string());
        }
    ...
    async fn load_profile_for_scope(
        ...
        let profiles = self.repo.find_profiles_for_scope(scope).await
            .map_err(ProfileServiceError::RepoError)?;
        ...
        let best_match = ProfileHierarchyPolicy::find_most_specific_profile(
            &profiles,
            game,
            car,
            track,
    ...
    fn create_cache_key(
        ...
        if let Some(overrides) = session_overrides {
            ...
            overrides.degrees_of_rotation.value().to_bits().hash(&mut hasher);
...
mod tests {
    ...
    impl ProfileRepo for MockProfileRepo {
        ...
        async fn find_profiles_for_scope(&self, scope: &ProfileScope) -> Result<Vec<Profile>, ProfileRepoError> {
            let profiles = self.profiles.read().await;
            let matching: Vec<Profile> = profiles.values()
        ...
        async fn load_global_profile(&self) -> Result<Profile, ProfileRepoError> {
            ...
            global.as_ref()
                .cloned()
                .ok_or_else(|| ProfileRepoError::ProfileNotFound(
    ...
    async fn test_profile_service_validation() {
        ...
        let validation_result = service.validate_profile(&profile, &capabilities).await.unwrap();
