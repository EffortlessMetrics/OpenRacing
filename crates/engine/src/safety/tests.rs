...
fn test_request_high_torque_success() {
    ...
    match service.state() {
        SafetyState::HighTorqueChallenge { challenge_token, ui_consent_given, .. } => {
...
fn test_request_high_torque_already_active() {
    ...
    let challenge = service.request_high_torque("test_device").unwrap();
    service.provide_ui_consent(challenge.challenge_token).unwrap();
    ...
    service.report_combo_start(challenge.challenge_token).unwrap();
...
fn test_ui_consent_flow() {
    ...
    match service.state() {
        SafetyState::AwaitingPhysicalAck { challenge_token, .. } => {
            assert_eq!(*challenge_token, challenge.challenge_token);
        }
...
fn test_hands_on_reset_timeout() {
    ...
    assert!(matches!(service.state(), SafetyState::HighTorqueActive { .. }));
}
...
fn test_multiple_devices() {
    ...
    service.provide_ui_consent(challenge1.challenge_token).unwrap();
    service.report_combo_start(challenge1.challenge_token).unwrap();
    std::thread::sleep(Duration::from_millis(2100));
...
fn test_fault_type_display() {
    ...
    assert_eq!(FaultType::HandsOffTimeout.to_string(), "Hands-off timeout exceeded");
    assert_eq!(FaultType::SafetyInterlockViolation.to_string(), "Safety interlock violation");
}
...
fn test_state_transitions() {
    ...
    service.request_high_torque("test_device").unwrap();
    assert!(matches!(service.state(), SafetyState::HighTorqueChallenge { .. }));
    ...
    assert!(matches!(service.state(), SafetyState::AwaitingPhysicalAck { .. }));
