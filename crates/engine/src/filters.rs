...
impl BumpstopState {
    pub fn new(enabled: bool, start_angle: f32, max_angle: f32, stiffness: f32, damping: f32) -> Self {
        Self {
            enabled,
            start_angle,
            max_angle,
...
pub fn bumpstop_filter(frame: &mut Frame, state: *mut u8) {
    unsafe {
        ...
        if abs_angle > state.start_angle {
            ...
            let bumpstop_penetration = (abs_angle - state.start_angle) / (state.max_angle - state.start_angle);
...
mod tests {
    ...
    fn test_notch_filter() {
        ...
        assert!(frame_dc.torque_out.is_finite(), "Notch filter output should be finite");
        assert!(frame_dc.torque_out.abs() < 10.0, "Notch filter should be stable");
        assert!((frame_dc.torque_out - 1.0).abs() < 0.5, "DC should pass through with minimal change");
    }
    ...
    fn test_hands_off_detector() {
        ...
        for _ in 0..3000 { // 3 seconds at 1kHz
    ...
    fn test_filter_determinism() {
        ...
        for &input in &inputs {
            ...
            assert_eq!(frame1.torque_out, frame2.torque_out, 
                      "Filter not deterministic for input {}", input);
        }
    }
}