...
pub enum TorquePattern {
    ...
    SineWave { amplitude: f32, frequency_hz: f32, phase_offset: f32 },
    ...
    Ramp { start_value: f32, end_value: f32, duration: Duration },
...
impl TorquePattern {
    ...
    pub fn value_at(&self, time: Duration, _start_time: Instant) -> f32 {
        ...
        match self {
            ...
            TorquePattern::SineWave { amplitude, frequency_hz, phase_offset } => {
                amplitude * (2.0 * std::f32::consts::PI * frequency_hz * t_sec + phase_offset).sin()
            },
            
            TorquePattern::SquareWave { amplitude, frequency_hz, duty_cycle } => {
            ...
            TorquePattern::Ramp { start_value, end_value, duration } => {
...
impl RTLoopTestHarness {
    ...
    pub fn add_virtual_device(&mut self, device: VirtualDevice) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Run a test scenario
    pub async fn run_scenario(&mut self, scenario: TestScenario) -> Result<TestResult, Box<dyn std::error::Error>> {
        ...
        let rt_handle = self.start_rt_loop(device.as_mut(), &scenario, start_time).await?;
        ...
        if performance.missed_tick_rate() > self.config.max_missed_tick_rate {
            errors.push(format!(
                ...
                performance.missed_tick_rate(), self.config.max_missed_tick_rate
            ));
            passed = false;
        }
        ...
        if result.passed {
            ...
            warn!("Test scenario '{}' FAILED: {:?}", scenario.name, result.errors);
        }
    ...
    async fn start_rt_loop(
        ...
        start_time: Instant,
    ) -> Result<tokio::task::JoinHandle<Result<(), Box<dyn std::error::Error + Send + Sync>>>, Box<dyn std::error::Error>> {
        let tick_interval = Duration::from_nanos((1_000_000_000.0 / self.config.update_rate_hz) as u64);
        ...
        let handle = tokio::spawn(async move {
            ...
            info!("RT loop stopped after {} ticks", tick_counter.load(Ordering::Relaxed));
            Ok(())
        });
...
mod tests {
    ...
    async fn test_torque_patterns() {
        ...
        assert_eq!(constant.value_at(Duration::from_secs(1), Instant::now()), 10.0);
