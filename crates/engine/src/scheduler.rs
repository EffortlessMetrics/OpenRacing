...
impl AbsoluteScheduler {
    ...
    fn sleep_until(&self, target: Instant) -> RTResult {
        ...
        unsafe {
            let timer = CreateWaitableTimerW(None, true, None)
                .map_err(|_| RTError::TimingViolation)?;
            ...
            SetWaitableTimer(timer, &due_time.dwLowDateTime as *const u32 as *const i64, 0, None, None, false)
                .map_err(|_| RTError::TimingViolation)?;
    ...
    fn apply_linux_rt_setup(&self, setup: &RTSetup) -> RTResult {
        use libc::{sched_setscheduler, sched_param, SCHED_FIFO, mlockall, MCL_CURRENT, MCL_FUTURE};
