...
impl TorqueCommand {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let cmd: Self = unsafe { mem::transmute([
        ...
        if cmd.report_id != report_ids::TORQUE_COMMAND {
            return Err(format!("Invalid report ID: expected 0x{:02x}, got 0x{:02x}", 
                report_ids::TORQUE_COMMAND, cmd.report_id));
        }
...
impl DeviceTelemetryReport {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let report: Self = unsafe { mem::transmute([
        ...
        if report.report_id != report_ids::DEVICE_TELEMETRY {
            return Err(format!("Invalid report ID: expected 0x{:02x}, got 0x{:02x}", 
                report_ids::DEVICE_TELEMETRY, report.report_id));
        }
...
impl DeviceCapabilitiesReport {
    ...
    pub fn new(
        ...
        let mut flags = 0u8;
        if supports_pid { flags |= Self::SUPPORTS_PID; }
        if supports_raw_torque_1khz { flags |= Self::SUPPORTS_RAW_TORQUE_1KHZ; }
        if supports_health_stream { flags |= Self::SUPPORTS_HEALTH_STREAM; }
        if supports_led_bus { flags |= Self::SUPPORTS_LED_BUS; }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        if report.report_id != report_ids::CAPABILITIES {
            return Err(format!("Invalid report ID: expected 0x{:02x}, got 0x{:02x}", 
                report_ids::CAPABILITIES, report.report_id));
        }
...
mod tests {
    ...
    fn test_fault_flags() {
        ...
        assert_eq!(report.faults & fault_flags::THERMAL_FAULT, fault_flags::THERMAL_FAULT);
        assert_eq!(report.faults & fault_flags::ENCODER_FAULT, fault_flags::ENCODER_FAULT);
