...
//! use racing_wheel_service::crypto::{TrustLevel, ed25519::PublicKey};
//!
//! // Create a file-backed trust store
//! let mut store = TrustStore::new("trust_store.json".into())?;
//!
//! // Add a trusted key
//! let public_key = PublicKey::from_bytes([0u8; 32], "my-key".to_string());
//! store.add_key(public_key, TrustLevel::Trusted, Some("My trusted key".to_string()))?;
//!
//! // Query trust level
...
mod tests {
    ...
    fn test_cannot_remove_system_key() -> Result<()> {
        ...
        // Find the system key fingerprint
        ...
        // Create a mutable store to test removal
        let mut store = TrustStore::new_in_memory();
    ...
    fn test_cannot_modify_system_key() -> Result<()> {
        ...
        // Create a mutable store to test modification
        let mut store = TrustStore::new_in_memory();
    ...
    fn test_list_keys() -> Result<()> {
        ...
        // Should have default key(s) plus our 3 test keys
        assert!(keys.len() >= 4);
    ...
    fn test_get_stats() -> Result<()> {
        ...
        let stats = store.get_stats();
        // At least 2 trusted (1 system + 1 added)
    ...
    fn test_file_backed_store() -> Result<()> {
        ...
        // Create a new store and add a key
        {
        ...
        // Reload the store and verify the key persists
        {
            ...
            let fingerprint =
                super::super::ed25519::Ed25519Verifier::get_key_fingerprint(&test_key);
    ...
    fn test_import_export_keys() -> Result<()> {
        ...
        // Export keys (excluding system keys)
    ...
    fn test_import_with_overwrite() -> Result<()> {
        ...
        // Create source store with a key
        let mut source_store = TrustStore::new_in_memory();
        let test_key = PublicKey {
            key_bytes: [50u8; 32],
        ...
        // Create destination store with same key but different trust level
        let mut dest_store = TrustStore::new_in_memory();
        dest_store.add_key(
            test_key,
            TrustLevel::Distrusted,
    ...
    fn test_export_with_system_keys() -> Result<()> {
        ...
        // Export with system keys should have more entries
