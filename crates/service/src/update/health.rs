...
pub async fn run_health_check(check: &HealthCheck) -> Result<()> {
    ...
    let result = timeout(timeout_duration, execute_health_check(check)).await
        .map_err(|_| anyhow::anyhow!("Health check timed out after {} seconds", check.timeout_seconds))?;
...
async fn execute_health_check(check: &HealthCheck) -> Result<()> {
    match &check.check_type {
        HealthCheckType::ServiceStart => {
            check_service_start().await
        }
        
        HealthCheckType::ServicePing => {
            check_service_ping().await
        }
        
        HealthCheckType::DeviceEnumeration => {
            check_device_enumeration().await
        }
        
        HealthCheckType::Command { command, args, expected_exit_code } => {
            check_command(command, args, *expected_exit_code).await
        }
        
        HealthCheckType::FileCheck { path, expected_hash, expected_size } => {
...
async fn check_command(command: &str, args: &[String], expected_exit_code: i32) -> Result<()> {
    ...
    if actual_exit_code != expected_exit_code {
        ...
        return Err(anyhow::anyhow!(
            ...
            command, actual_exit_code, expected_exit_code, stderr
        ));
    }
...
async fn check_file(
    ...
    if let Some(expected_hash) = expected_hash {
        let actual_hash = crate::update::delta::compute_file_hash(path).await
...
impl HealthCheckRunner {
    ...
    pub async fn run_checks_with_summary(&self, checks: &[HealthCheck]) -> Result<HealthCheckSummary> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_file_check_success() {
        let mut temp_file = NamedTempFile::new().unwrap();
        tokio::fs::write(temp_file.path(), b"test content").await.unwrap();
    ...
    #[tokio::test]
    async fn test_file_check_size_mismatch() {
        let mut temp_file = NamedTempFile::new().unwrap();
        tokio::fs::write(temp_file.path(), b"test content").await.unwrap();
