...
pub async fn perform_rollback(
    ...
    for file_path in &metadata.files {
        ...
        if backup_file_path.exists() {
            ...
            // Restore the file
            fs::copy(&backup_file_path, &target_file_path).await
                .with_context(|| format!(
...
async fn load_backup_metadata(metadata_path: &Path) -> Result<BackupMetadata> {
    ...
    serde_json::from_str(&content)
        .context("Failed to parse backup metadata JSON")
}
    ...
    if let Ok(mut entries) = fs::read_dir(&backup_path).await {
        while let Ok(Some(entry)) = entries.next_entry().await {
            ...
            if entry_path.is_file() {
                let relative_path = entry_path.strip_prefix(&backup_path)
...
impl RollbackManager {
    ...
    pub async fn get_backup_info(&self) -> Result<Vec<BackupInfo>> {
        ...
        for backup in backups {
            let size = get_backup_size(&self.backup_dir, &backup.backup_id).await
                .unwrap_or(0);
            
            let verification = verify_backup_integrity(&self.backup_dir, &backup.backup_id).await
