...
//!
//! # Modules
//! - `firmware`: Firmware update system with A/B partition support
//! - `delta`: Delta update compression and patching
//! - `rollback`: Rollback functionality for failed updates
//! - `health`: Health check system for update validation
//! - `staged_rollout`: Staged rollout for gradual deployments
...
pub use firmware::{
    CachedFirmware, FfbBlocker, FirmwareCache, FirmwareDevice, FirmwareImage, FirmwareUpdateError,
    FirmwareUpdateManager, Partition, PartitionHealth, PartitionInfo, StagedRolloutConfig,
...
impl UpdateManager {
    ...
    pub async fn apply_update(&mut self, package_path: &Path) -> Result<UpdateResult> {
        ...
        let package = self
            .load_and_verify_package(package_path)
            .await
            .context("Failed to load update package")?;

        tracing::info!(
            "Loaded update package for version {}",
        ...
        self.run_health_checks(&package.pre_checks, "pre-update")
            .await
        ...
        match apply_result {
            Ok(()) => {
                ...
                match self
                    .run_health_checks(&package.post_checks, "post-update")
                    .await
                {
                    Ok(()) => {
    ...
    async fn create_backup(&self, package: &UpdatePackage) -> Result<String> {
        ...
        for file in &package.files {
            if matches!(
                file.operation,
                FileOperation::Replace { .. } | FileOperation::Delta { .. }
            ) {
                ...
                if source_path.exists() {
                    ...
                    // Copy file to backup
                    tokio::fs::copy(&source_path, &backup_file_path)
                        .await
    ...
    async fn apply_package_files(&self, package: &UpdatePackage) -> Result<()> {
        ...
        for file in &package.files {
            self.apply_file_operation(file).await.with_context(|| {
                format!(
                    "Failed to apply operation for file: {}",
                    file.path.display()
                )
            })?;
        }
        ...
        match &file.operation {
            FileOperation::Replace { data } => {
                ...
                let decompressed_data =
                    delta::decompress_data(data).context("Failed to decompress file data")?;
                ...
                // Verify file hash
                let actual_hash = delta::compute_file_hash(&target_path)
                    .await
            ...
            FileOperation::Delta { patch } => {
                // Apply binary delta patch
                delta::apply_delta_patch(&target_path, patch)
                    .await
    ...
    async fn rollback_update(&self, backup_id: &str) -> Result<()> {
        ...
        rollback::perform_rollback(&self.backup_dir, &self.install_dir, backup_id)
            .await
    ...
    async fn cleanup_old_backups(&self) -> Result<()> {
        rollback::cleanup_old_backups(&self.backup_dir, 7)
            .await // Keep 7 days of backups
