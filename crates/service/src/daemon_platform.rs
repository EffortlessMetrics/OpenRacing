...
impl ServiceDaemon {
    ...
    pub(crate) async fn uninstall_windows_service() -> Result<()> {
        ...
        // Stop service first
        let _ = Command::new("sc").args(["stop", "wheeld"]).output();
...
impl ServiceDaemon {
    pub(crate) async fn install_unix_service() -> Result<()> {
        ...
        let systemd_dir = PathBuf::from(&home_dir).join(".config/systemd/user");
...
"#,
        ...
        tokio::fs::write(&service_file, service_content)
            .await
    ...
    pub(crate) async fn uninstall_unix_service() -> Result<()> {
        ...
        let service_file = PathBuf::from(&home_dir).join(".config/systemd/user/wheeld.service");

        if service_file.exists() {
            tokio::fs::remove_file(&service_file)
                .await
