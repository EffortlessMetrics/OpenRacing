...
mod tests {
    use super::*;
    use crate::{WheelService, ApplicationProfileService, ApplicationDeviceService, ApplicationSafetyService};
    use racing_wheel_engine::{VirtualHidPort, SafetyPolicy, MockProfileRepo, TracingManager};
    use racing_wheel_schemas::{DeviceId, TorqueNm, Profile, FFBSettings};
    use std::sync::Arc;
    ...
    async fn test_service_integration_workflow() {
        ...
        if let Ok(profile_id) = service.profile_service().create_profile(profile).await {
            ...
            let apply_result = service.profile_service()
                .apply_profile_to_device(&device_id, &profile_id, context).await;
        ...
        assert!(unregister_result.is_ok(), "Device unregistration should succeed");
    }
    ...
    async fn test_error_handling_scenarios() {
        ...
        let result = service.profile_service().create_profile(invalid_profile).await;
        ...
        let result = service.device_service().get_device(&nonexistent_device).await;
        assert!(result.is_ok(), "get_device should return Ok(None) for nonexistent device");
        assert!(result.unwrap().is_none(), "Should return None for nonexistent device");
    }
    ...
    async fn test_service_statistics() {
        ...
        let profile_stats = service.profile_service().get_profile_statistics().await.unwrap();
    ...
    async fn test_concurrent_service_operations() {
        ...
        assert!(result3.unwrap().is_ok(), "Safety registration should succeed");
    }
    ...
    async fn test_service_timeout_handling() {
        ...
        assert!(device_enumeration.is_ok(), "Device enumeration should not timeout");
        ...
        assert!(safety_registration.unwrap().is_ok(), "Safety registration should succeed");
    }
