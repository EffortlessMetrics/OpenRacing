...
impl ConfigValidationService {
    ...
    fn load_golden_files() -> HashMap<String, GoldenFileFixture> {
        ...
        fixtures.insert("iracing".to_string(), GoldenFileFixture {
        ...
        fixtures.insert("acc".to_string(), GoldenFileFixture {
  ...
  "updateRateHz": 100
            ...
            expected_files: vec![
                ExpectedFile {
                    path: "Documents/Assetto Corsa Competizione/Config/broadcasting.json".to_string(),
                    content: r#"{
  "updListenerPort": 9996,
  "connectionId": "",
  "broadcastingPort": 9000,
  ...
  "updateRateHz": 100
    ...
    pub async fn validate_config_generation(
        ...
        let fixture = self.golden_files.get(game_id)
        ...
        for expected_diff in expected_diffs {
            ...
            if let Some(actual_diff) = actual_diffs.iter().find(|d| {
                d.file_path == expected_diff.file_path && d.key == expected_diff.key
            }) {
        ...
        for actual_diff in actual_diffs {
            ...
            if !expected_diffs.iter().any(|d| {
                d.file_path == actual_diff.file_path && d.key == actual_diff.key
            }) {
    ...
    pub async fn validate_led_heartbeat(&self) -> Result<ValidationResult> {
        ...
        let success = match validation_result {
            Ok(Ok(heartbeat_count)) => {
                ...
                details.matched_items.push(format!("heartbeat_cycles:{}", heartbeat_count));
                
                heartbeat_count >= (self.led_validation.expected_pattern.repeat_count as usize)
            }
            Ok(Err(e)) => {
                details.errors.push(format!("LED heartbeat check failed: {}", e));
                false
            }
            Err(_) => {
                details.errors.push("LED heartbeat validation timed out".to_string());
                false
            }
        };
    ...
    pub async fn validate_end_to_end(
        ...
        let config_gen_result = self.validate_config_generation(game_id, actual_diffs).await?;
        ...
        details.matched_items.extend(config_gen_result.details.matched_items);
        details.matched_items.extend(config_file_result.details.matched_items);
        details.matched_items.extend(led_result.details.matched_items);
        
        details.missing_items.extend(config_gen_result.details.missing_items);
        details.missing_items.extend(config_file_result.details.missing_items);
        details.missing_items.extend(led_result.details.missing_items);
        ...
        let success = config_gen_result.success 
            && config_file_result.success 
            && led_result.success;
    ...
    async fn simulate_led_heartbeat_check(&self) -> Result<usize> {
        ...
        info!(heartbeat_count = heartbeat_count, "LED heartbeat simulation completed");
        Ok(heartbeat_count)
    }
...
mod tests {
    ...
    async fn test_config_generation_validation() {
        ...
        let result = service.validate_config_generation("iracing", &actual_diffs).await.unwrap();
    ...
    async fn test_config_file_validation() {
        ...
        let result = service.validate_config_files("iracing", temp_dir.path()).await.unwrap();
        assert!(result.success);
        assert_eq!(result.details.matched_items.len(), 1);
    }
