...
impl ACCAdapter {
    ...
    fn parse_udp_packet(data: &[u8]) -> Result<ACCTelemetryData> {
        if data.len() < std::mem::size_of::<ACCTelemetryData>() {
            return Err(anyhow::anyhow!(
                "ACC packet too small: {} bytes",
                data.len()
            ));
        }

        // ACC uses a simple binary format
        let telemetry_data = unsafe { std::ptr::read(data.as_ptr() as *const ACCTelemetryData) };
    ...
    fn normalize_acc_data(data: &ACCTelemetryData) -> NormalizedTelemetry {
        ...
        extended.insert(
            ...
            TelemetryValue::Float(data.fuel),
        );
        extended.insert(
            "lap_count".to_string(),
            TelemetryValue::Integer(data.completed_laps),
        );
        extended.insert(
            "current_lap_time".to_string(),
            TelemetryValue::Float(data.current_lap_time),
        );
        extended.insert(
            "last_lap_time".to_string(),
            TelemetryValue::Float(data.last_lap),
        );
        extended.insert(
            "best_lap_time".to_string(),
            TelemetryValue::Float(data.best_lap),
        );
        ...
        extended.insert(
            "steering_angle".to_string(),
            TelemetryValue::Float(data.steer_angle),
        );
        extended.insert(
            "tc_level".to_string(),
            TelemetryValue::Integer(data.tc as i32),
        );
        extended.insert(
            "abs_level".to_string(),
            TelemetryValue::Integer(data.abs as i32),
        );
        extended.insert(
            "ers_recovery".to_string(),
            TelemetryValue::Float(data.ers_recovery_level),
        );
        extended.insert(
            "ers_power".to_string(),
            TelemetryValue::Float(data.ers_power_level),
        );
