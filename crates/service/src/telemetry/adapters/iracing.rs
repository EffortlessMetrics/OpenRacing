...
impl IRacingAdapter {
    ...
    fn initialize_shared_memory(&mut self) -> Result<()> {
        ...
        unsafe {
            ...
            let data_ptr = MapViewOfFile(handle, FILE_SHARE_READ, 0, 0, 0) as *const IRacingData;
    ...
    fn initialize_shared_memory(&mut self) -> Result<()> {
        Err(anyhow::anyhow!(
            "iRacing shared memory only available on Windows"
        ))
    }
    ...
    async fn check_iracing_running(&self) -> bool {
        ...
        unsafe {
            let handle = OpenFileMappingW(FILE_SHARE_READ, 0, memory_name.as_ptr() as *const u16);
...
impl TelemetryAdapter for IRacingAdapter {
    ...
    fn normalize(&self, raw: &[u8]) -> Result<NormalizedTelemetry> {
        ...
        let data: IRacingData = unsafe { ptr::read(raw.as_ptr() as *const IRacingData) };
