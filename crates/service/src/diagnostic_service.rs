    ...
    async fn run(&self, _system_info: &SystemInfo) -> Result<DiagnosticResult> {
        ...
        #[cfg(target_os = "linux")]
        {
            let hidraw_count = std::fs::read_dir("/dev")?
                ...
                .filter(|entry| {
                    entry.file_name().to_string_lossy().starts_with("hidraw")
                })
                .count();
        ...
        match hidapi::HidApi::new() {
            Ok(api) => {
                ...
                let wheel_devices: Vec<_> = devices.iter()
                    .filter(|device| Self::is_racing_wheel_device(device))
                    .collect();
                
                metadata.insert("racing_wheel_devices".to_string(), wheel_devices.len().to_string());
...
impl DiagnosticTest for RealtimeCapabilityTest {
    ...
    async fn run(&self, _system_info: &SystemInfo) -> Result<DiagnosticResult> {
        ...
        let message = "Real-time capabilities available".to_string();
        ...
        #[cfg(target_os = "linux")]
        {
                ...
                .await
            {
                ...
                if !rtkit_active {
                    ...
                    suggested_actions.push("Install and enable rtkit for real-time scheduling".to_string());
                }
            }
            ...
            if let Ok(limits) = std::fs::read_to_string("/proc/self/limits") {
                if limits.contains("Max locked memory") {
                    ...
                    suggested_actions.push("Configure memlock limits for real-time operation".to_string());
                }
            }
        }
...
impl DiagnosticTest for MemoryTest {
    ...
    async fn run(&self, system_info: &SystemInfo) -> Result<DiagnosticResult> {
        ...
        metadata.insert("total_memory_mb".to_string(), system_info.memory_mb.to_string());
        ...
        let message = if available_mb < 512 {
            ...
            format!("Memory status: {} MB available of {} MB total", available_mb, system_info.memory_mb)
        };
    ...
    async fn run(&self, _system_info: &SystemInfo) -> Result<DiagnosticResult> {
            ...
            if let Some(config_dir) = config_path.parent() {
                match std::fs::metadata(config_dir) {
                    Ok(meta) => {
                        ...
                        metadata.insert("config_dir_writable".to_string(), (!meta.permissions().readonly()).to_string());
                    }
                    Err(_) => {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Ok(entries) = std::fs::read_dir("/dev") {
                ...
                metadata.insert("hidraw_accessible".to_string(), accessible_count.to_string());
...
impl DiagnosticTest for NetworkTest {
    ...
    async fn run(&self, _system_info: &SystemInfo) -> Result<DiagnosticResult> {
        ...
        let message = format!("Network test: {}/{} test ports available", bindable_ports, test_ports.len());
...
impl DiagnosticTest for GameIntegrationTest {
    ...
    async fn run(&self, _system_info: &SystemInfo) -> Result<DiagnosticResult> {
        ...
        for (game_name, paths) in &game_checks {
            ...
            metadata.insert(format!("{}_detected", game_name.to_lowercase()), found.to_string());
            if found {
...
impl DiagnosticTest for SafetySystemTest {
    ...
    async fn run(&self, _system_info: &SystemInfo) -> Result<DiagnosticResult> {
        ...
        for fault_type in &fault_types {
            metadata.insert(format!("fault_{}_handler", fault_type), "available".to_string());
        }
