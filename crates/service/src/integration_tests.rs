...
mod tests {
    use super::*;
    use crate::{WheelService, SystemConfig, FeatureFlags, ServiceDaemon};
    use racing_wheel_engine::{VirtualHidPort, MockTelemetrySource};
    use std::sync::Arc;
    ...
    async fn test_complete_system_startup_shutdown() {
        ...
        let daemon = ServiceDaemon::new_with_flags(
            crate::ServiceConfig::from_system_config(&config),
            flags
        ).await.expect("Failed to create service daemon");
    ...
    async fn test_device_enumeration_and_management() {
        ...
        let devices = service.device_service().enumerate_devices().await
    ...
    async fn test_profile_management() {
        ...
        // Save profile
        let save_result = service.profile_service()
            .save_profile(&test_profile).await;
        assert!(save_result.is_ok(), "Failed to save profile");
        
        // Load profile
        let loaded_profile = service.profile_service()
            .load_profile(&test_profile.scope).await;
        assert!(loaded_profile.is_ok(), "Failed to load profile");
        
        // Apply profile
        let apply_result = service.profile_service()
            .apply_profile(&test_profile).await;
        assert!(apply_result.is_ok(), "Failed to apply profile");
        
        // Verify profile is active
        let active_profiles = service.profile_service()
            .get_active_profiles().await;
    ...
    async fn test_safety_system() {
        ...
        assert!(matches!(safety_state.state, racing_wheel_engine::SafetyState::SafeTorque));
        ...
        if let Some(device) = devices.first() {
            ...
            let high_torque_result = service.safety_service()
                .request_high_torque(&device.id).await;
            assert!(high_torque_result.is_err(), "High torque should be denied without unlock");
            
            // Test fault injection and recovery
            let fault_result = service.safety_service()
                .inject_test_fault(&device.id, racing_wheel_engine::FaultType::ThermalLimit).await;
            assert!(fault_result.is_ok(), "Failed to inject test fault");
            
            // Verify fault state
            let safety_state = service.safety_service().get_safety_state().await;
            assert!(matches!(safety_state.state, racing_wheel_engine::SafetyState::Faulted { .. }));
            
            // Test fault recovery
            let recovery_result = service.safety_service()
                .clear_fault(&device.id).await;
    ...
    async fn test_ipc_communication() {
        let config = create_test_system_config().await;
        let service_config = crate::ServiceConfig::from_system_config(&config);
        
        // Create IPC server
        let ipc_server = crate::IpcServer::new(service_config.ipc.clone()).await
        ...
        let server_handle = tokio::spawn(async move {
            ipc_server.serve(Arc::new(service)).await
        });
        ...
        // Create IPC client
        let client = crate::IpcClient::new(service_config.ipc.clone()).await
    ...
    async fn test_plugin_system() {
        ...
        if let Some(plugin) = plugins.first() {
            ...
            let test_telemetry = racing_wheel_engine::NormalizedTelemetry {
                ...
                slip_ratio: 0.1,
                gear: 3,
                flags: racing_wheel_engine::TelemetryFlags::empty(),
    ...
    async fn test_anticheat_compatibility() {
        ...
        let report = crate::AntiCheatReport::generate().await
            .expect("Failed to generate anti-cheat report");
        
        // Verify key compatibility points
        assert!(!report.process_info.dll_injection, "Should not use DLL injection");
        assert!(report.process_info.kernel_drivers.is_empty(), "Should not use kernel drivers");
        
        // Verify all telemetry methods are documented
        assert!(!report.telemetry_methods.is_empty(), "Should document telemetry methods");
        
        for method in &report.telemetry_methods {
            assert!(method.anticheat_compatible, "All telemetry methods should be anti-cheat compatible");
            assert!(!method.compatibility_notes.is_empty(), "Should have compatibility notes");
        }
        
        // Verify security measures
        assert!(!report.security_measures.is_empty(), "Should document security measures");
        
        // Generate markdown report
        let markdown = report.to_markdown();
        assert!(markdown.contains("Anti-Cheat Compatibility Report"), "Should contain report title");
        assert!(markdown.contains("No DLL Injection"), "Should document no DLL injection");
        assert!(markdown.contains("No Kernel Drivers"), "Should document no kernel drivers");
    }

    // Helper functions

    async fn create_test_service() -> WheelService {
        WheelService::new_with_flags(create_test_feature_flags()).await
