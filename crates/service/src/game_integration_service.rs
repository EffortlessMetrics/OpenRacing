...
use crate::game_service::{ConfigDiff, GameService};
...
impl GameIntegrationService {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        let auto_switching_service =
            AutoProfileSwitchingService::new(self.profile_service.clone())?;
    ...
    pub async fn configure_one_click(
        &self,
        request: OneClickConfigRequest,
    ) -> Result<OneClickConfigResult> {
        ...
        if request.enable_auto_switching
            && success
            && let Some(profile_id) = &request.profile_id
        {
            match self
                .setup_auto_switching(&request.game_id, profile_id)
                .await
            {
                Ok(()) => {
        ...
        let _ = self
            .event_sender
            .send(GameIntegrationEvent::GameConfigured {
                game_id: request.game_id.clone(),
                success,
    ...
    async fn setup_auto_switching(&self, game_id: &str, profile_id: &str) -> Result<()> {
        ...
        if let Some(auto_switching) = auto_switching_guard.as_ref() {
            auto_switching
                .set_game_profile(game_id.to_string(), profile_id.to_string())
                .await?;
            info!(
                game_id = %game_id,
                profile_id = %profile_id,
    ...
    pub async fn validate_configuration(
        ...
        let _ = self
            .event_sender
            .send(GameIntegrationEvent::ValidationCompleted {
