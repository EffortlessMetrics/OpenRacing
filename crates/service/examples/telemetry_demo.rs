...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ✓ Created telemetry: FFB={:.2}, RPM={:.0}, Speed={:.1} m/s, Gear={}", 
    ...
    println!("   ✓ Rate limiter protected RT thread: {} processed, {} dropped ({:.1}% drop rate)",
        processed, dropped, rate_limiter.drop_rate_percent());
    ...
    for i in 0..5 {
        if let Ok(frame) = tokio::time::timeout(Duration::from_millis(100), receiver.recv()).await {
            if let Some(frame) = frame {
                println!("   📊 Frame {}: RPM={:.0}, Speed={:.1} m/s, FFB={:.2}", 
                    i + 1,
    ...
    println!("   ✓ Recorded {} frames to {}", recording.frames.len(), recording_path.display());
    ...
    while let Some(frame) = player.get_next_frame() {
        replayed_count += 1;
        if replayed_count <= 3 {
            println!("   📊 Replay {}: RPM={:.0}, Speed={:.1} m/s", 
                replayed_count,
                frame.data.rpm.unwrap_or(0.0),
                frame.data.speed_ms.unwrap_or(0.0)
            );
    ...
    for (name, scenario) in test_scenarios {
        ...
        println!("   ✓ Generated {} scenario: {} frames", name, recording.frames.len());
    }
    ...
    println!("   ✓ Adaptive rate limiting: High CPU={}Hz, Low CPU={}Hz", high_cpu_rate, low_cpu_rate);
    ...
    println!("   ✓ Flags active: {}", extended_telemetry.has_active_flags());
    println!("   ✓ Extended data fields: {}", extended_telemetry.extended.len());
    ...
    println!("   ✓ iRacing adapter handles invalid data: {}", iracing_result.is_err());
    println!("   ✓ ACC adapter handles invalid data: {}", acc_result.is_err());
