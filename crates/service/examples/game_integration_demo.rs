...
use racing_wheel_service::{GameIntegrationService, OneClickConfigRequest, ProfileService};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Demo 4: Comprehensive E2E tests (disabled - uses test-only types)
...
async fn demo_one_click_configuration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let iracing_result = integration_service
        .configure_one_click(iracing_request)
        .await?;
    info!(
...
async fn demo_performance_testing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match switch_result {
        Ok(duration) => {
            ...
            if duration_ms <= 500 {
                ...
                info!(
                    "✗ Profile switching exceeds 500ms requirement: {}ms",
                    duration_ms
                );
            }
        }
    ...
    if config_duration.as_millis() < 1000 {
        ...
        info!(
            "✗ Configuration performance is slow: {}ms",
            config_duration.as_millis()
        );
    }
...
// Disabled: uses test-only types (GameIntegrationE2ETestSuite)
