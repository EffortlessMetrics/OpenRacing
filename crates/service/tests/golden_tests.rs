...
use racing_wheel_service::config_writers::{IRacingConfigWriter, ACCConfigWriter};
use std::fs;
use std::path::Path;
...
async fn test_game_support_matrix_structure() {
    ...
    assert!(iracing_support.versions[0].config_paths.contains(&"Documents/iRacing/app.ini".to_string()));
    assert!(iracing_support.versions[0].executable_patterns.contains(&"iRacingSim64DX11.exe".to_string()));
    
    // Verify auto-detection config
    assert!(iracing_support.auto_detect.process_names.contains(&"iRacingSim64DX11.exe".to_string()));
    assert!(iracing_support.auto_detect.install_registry_keys.contains(&"HKEY_CURRENT_USER\\Software\\iRacing.com\\iRacing".to_string()));
    ...
    assert!(acc_support.versions[0].config_paths.contains(&"Documents/Assetto Corsa Competizione/Config/broadcasting.json".to_string()));
}
...
async fn test_configuration_diff_generation() {
    ...
    let iracing_diffs = service.get_expected_diffs("iracing", &iracing_config).await.unwrap();
    ...
    let acc_diffs = service.get_expected_diffs("acc", &acc_config).await.unwrap();
...
async fn test_unsupported_game_handling() {
    ...
    let config_result = service.get_expected_diffs("unsupported_game", &TelemetryConfig {
        enabled: true,
        update_rate_hz: 60,
    ...
    assert!(config_result.unwrap_err().to_string().contains("No config writer for game"));
}
...
async fn test_end_to_end_telemetry_configuration() {
    ...
    let iracing_diffs = service.configure_telemetry("iracing", temp_dir.path()).await.unwrap();
    assert_eq!(iracing_diffs.len(), 1);
    assert_eq!(iracing_diffs[0].key, "telemetryDiskFile");
    assert_eq!(iracing_diffs[0].new_value, "1");
    
    // Test ACC end-to-end configuration
    let acc_diffs = service.configure_telemetry("acc", temp_dir.path()).await.unwrap();
    assert_eq!(acc_diffs.len(), 1);
    assert_eq!(acc_diffs[0].key, "entire_file");
