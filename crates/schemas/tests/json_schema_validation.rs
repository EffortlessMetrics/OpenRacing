...
fn test_profile_round_trip_serialization() {
    ...
    assert_eq!(
        ...
        deserialized_profile.base.filters.reconstruction
    );
...
fn test_filter_config_required_fields() {
    ...
    assert!(
        json_obj.contains_key("bumpstop"),
        "FilterConfig should contain bumpstop field"
    );
    assert!(
        json_obj.contains_key("handsOff"),
        "FilterConfig should contain handsOff field"
    );
    assert!(
