...
fn defaults_are_stable_at_1khz() {
    ...
    assert_eq!(config.slew_rate, 1.0, "Slew rate should be 1.0 (no limiting) for stability");
    
    // Verify explicit torque cap for test predictability
    assert_eq!(config.torque_cap, Some(10.0), "Torque cap should be Some(10.0) for test predictability");
    ...
    assert!(config.bumpstop.enabled, "Bumpstop should be enabled by default");
    assert_eq!(config.bumpstop.strength, 0.5, "Bumpstop strength should use default");
    
    assert!(config.hands_off.enabled, "Hands-off detection should be enabled by default");
    assert_eq!(config.hands_off.sensitivity, 0.3, "Hands-off sensitivity should use default");
}
...
fn filter_config_stability_properties() {
    ...
    assert!((input_range - 1.0).abs() < f32::EPSILON, "Input range should be 1.0");
    assert!((output_range - 1.0).abs() < f32::EPSILON, "Output range should be 1.0");
    assert!((curve[0].input - 0.0).abs() < f32::EPSILON, "Curve starts at 0.0");
    assert!((curve[0].output - 0.0).abs() < f32::EPSILON, "Curve output starts at 0.0");
    
    // No frequency-based filtering
    assert!(config.notch_filters.is_empty(), "No notch filters that could cause resonance");
}