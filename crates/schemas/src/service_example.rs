...
mod example {
    ...
    impl MockIpcService {
        ...
        fn get_device_status_ipc(&self, wire_device_id: proto::DeviceId) -> Option<proto::DeviceStatus> {
        ...
        fn update_telemetry_ipc(&mut self, wire_device_id: proto::DeviceId, wire_telemetry: proto::TelemetryData) -> Result<(), String> {
            // Convert wire types to domain types
            let device_id: DeviceId = wire_device_id.id.parse()
                .map_err(|e| format!("Invalid device ID: {}", e))?;
            
            let telemetry: TelemetryData = wire_telemetry.try_into()
