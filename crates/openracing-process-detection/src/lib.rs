...
impl ProcessDetectionService {
    ...
    pub async fn start_monitoring(&mut self) -> Result<()> {
        ...
        let mut ticker = interval(self.detection_interval);
        ...
        loop {
            ...
            match self.scan_processes().await {
                ...
                Err(error) => {
                    warn!(error = %error, "Failed to scan processes");
                }
            }
        }
    }
    ...
    async fn update_process_state(&mut self, current_processes: Vec<ProcessInfo>) {
        ...
        let currently_running_games: Vec<String> = self
            .running_processes
            .values()
            .filter_map(|process| process.game_id.as_ref().map(ToString::to_string))
            .collect();
    ...
    fn is_game_currently_running(&self, game_id: &str) -> bool {
        self.running_processes
            .values()
            .any(|process| process.game_id.as_deref() == Some(game_id))
    }

    fn get_game_process_info(&self, game_id: &str) -> Option<ProcessInfo> {
        self.running_processes
            .values()
            .find(|process| process.game_id.as_deref() == Some(game_id))
            .cloned()
    }
