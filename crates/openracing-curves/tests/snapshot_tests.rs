...
#[test]
fn snapshot_exponential_05_lut() {
    let curve = CurveType::exponential(0.5).unwrap();
    let lut = curve.to_lut();
    let snapshot = normalize_lut_for_snapshot(&lut);
...
#[test]
fn snapshot_logarithmic_10_lut() {
...
#[test]
fn snapshot_logarithmic_e_lut() {
    let curve = CurveType::logarithmic(std::f32::consts::E).unwrap();
    let lut = curve.to_lut();
    let snapshot = normalize_lut_for_snapshot(&lut);
...
#[test]
fn snapshot_bezier_s_curve_lut() {
    let curve = BezierCurve::new([(0.0, 0.0), (0.25, 0.75), (0.75, 0.25), (1.0, 1.0)]).unwrap();
    let lut = curve.to_lut();
    let snapshot = normalize_lut_for_snapshot(&lut);
...
#[test]
fn snapshot_bezier_aggressive_lut() {
    let curve = BezierCurve::new([(0.0, 0.0), (0.1, 0.9), (0.9, 0.1), (1.0, 1.0)]).unwrap();
    let lut = curve.to_lut();
    let snapshot = normalize_lut_for_snapshot(&lut);
