...
fn make_forza_sled() -> Vec<u8> {
    ...
    write_f32(&mut data, 32, 40.0); // vel_x → speed
    data
}
...
fn make_beamng() -> Vec<u8> {
    ...
    write_f32(&mut data, 16, 6000.0); // rpm
    write_f32(&mut data, 48, 0.8); // throttle
    write_f32(&mut data, 52, 0.0); // brake
    data
}
...
fn make_ets2() -> Vec<u8> {
    ...
    write_f32(&mut data, 4, 25.0); // speed_ms
    write_f32(&mut data, 8, 1800.0); // engine_rpm
    write_i32(&mut data, 12, 6); // gear = 6
    write_f32(&mut data, 16, 0.7); // fuel_ratio
    write_f32(&mut data, 20, 0.55); // engine_load
    data
}
...
fn make_pcars2() -> Vec<u8> {
    ...
    data[29] = 0; // brake u8
    write_f32(&mut data, 36, 45.0); // speed f32 m/s
...
fn make_dirt3() -> Vec<u8> {
    ...
    write_f32(&mut buf, 100, 28.0); // wheel_speed_rl
    write_f32(&mut buf, 104, 28.0); // wheel_speed_rr
    write_f32(&mut buf, 108, 29.0); // wheel_speed_fl
    write_f32(&mut buf, 112, 29.0); // wheel_speed_fr
    write_f32(&mut buf, 116, 0.7); // throttle
    write_f32(&mut buf, 120, -0.1); // steer
    write_f32(&mut buf, 124, 0.0); // brake
    write_f32(&mut buf, 132, 3.0); // gear
    write_f32(&mut buf, 148, 5500.0); // rpm
    write_f32(&mut buf, 252, 8000.0); // max_rpm
    buf
}
...
fn make_wreckfest() -> Vec<u8> {
    ...
    write_f32(&mut buf, 8, 38.0); // speed_ms
    write_f32(&mut buf, 12, 5000.0); // rpm
    buf[16] = 3; // gear
...
fn make_dakar() -> Vec<u8> {
    ...
    write_f32(&mut data, 8, 32.0); // speed_ms
    write_f32(&mut data, 12, 4500.0); // rpm
    data[16] = 3; // gear
    write_f32(&mut data, 20, 0.3); // lateral_g
    write_f32(&mut data, 24, 0.2); // longitudinal_g
    write_f32(&mut data, 28, 0.7); // throttle
    write_f32(&mut data, 32, 0.0); // brake
    write_f32(&mut data, 36, 0.15); // steering_angle
    data
}
...
fn make_flatout() -> Vec<u8> {
    ...
    write_f32(&mut data, 8, 28.0); // speed_ms
    write_f32(&mut data, 12, 5000.0); // rpm
    data[16] = 3; // gear
...
fn make_rbr() -> Vec<u8> {
    ...
    write_f32(&mut buf, 12, 25.0); // speed_ms
    write_f32(&mut buf, 52, 0.75); // throttle
    write_f32(&mut buf, 56, 0.0); // brake
    write_f32(&mut buf, 64, 3.0); // gear
    write_f32(&mut buf, 68, -0.2); // steering
    write_f32(&mut buf, 116, 5500.0); // rpm
    buf
}
...
fn make_rennsport() -> Vec<u8> {
    ...
    write_f32(&mut buf, 4, 200.0); // speed_kmh
    write_f32(&mut buf, 8, 7500.0); // rpm
    buf[12] = 4; // gear
    write_f32(&mut buf, 16, 0.5); // ffb_scalar
    write_f32(&mut buf, 20, 0.1); // slip_ratio
    buf
}
...
fn make_nascar() -> Vec<u8> {
    ...
    write_f32(&mut data, 16, 55.0); // speed_ms
    write_f32(&mut data, 68, 4.0); // gear (float)
    write_f32(&mut data, 72, 7500.0); // rpm
    write_f32(&mut data, 80, 0.85); // throttle
    write_f32(&mut data, 84, 0.0); // brake
    write_f32(&mut data, 88, -0.1); // steer
    data
}
...
fn make_wrc_kylotonn() -> Vec<u8> {
    let mut data = vec![0u8; 96];
    write_f32(&mut data, 0, 0.5); // stage_progress
    write_f32(&mut data, 4, 30.0); // road_speed_ms
    write_f32(&mut data, 8, -0.2); // steering
    write_f32(&mut data, 12, 0.8); // throttle
    write_f32(&mut data, 16, 0.0); // brake
    write_u32(&mut data, 28, 3); // gear
    write_f32(&mut data, 32, 5500.0); // rpm
...
fn make_vrally4() -> Vec<u8> {
    let mut data = vec![0u8; 96];
    write_f32(&mut data, 4, 25.0); // speed_ms
    write_f32(&mut data, 8, 0.1); // steering
    write_f32(&mut data, 12, 0.65); // throttle
    write_f32(&mut data, 16, 0.0); // brake
    write_u32(&mut data, 28, 3); // gear
    write_f32(&mut data, 32, 5000.0); // rpm
...
fn make_raceroom() -> Vec<u8> {
    ...
    write_f32(&mut buf, 1396, rps); // engine_rps
    write_f32(&mut buf, 1400, max_rps); // max_engine_rps
    write_f32(&mut buf, 1392, 48.0); // car_speed m/s
    write_f32(&mut buf, 1500, 0.8); // throttle
