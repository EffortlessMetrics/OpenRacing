//! WTCR (World Touring Car Cup) telemetry adapter for Codemasters Mode 1 UDP format.
//!
//! Enable UDP telemetry in-game: Options → Controls → UDP Telemetry, port 6778.
//!
//! The packet layout is the fixed-layout Codemasters Mode 1 legacy binary stream
//! (264+ bytes, little-endian `f32` at known byte offsets), shared with GRID Autosport,
//! DiRT Rally 2.0, WRC Generations, and the broader Codemasters racing series.
...
const DEFAULT_PORT: u16 = 6778;
const MIN_PACKET_SIZE: usize = 264;
const MAX_PACKET_SIZE: usize = 2048;
const DEFAULT_HEARTBEAT_TIMEOUT_MS: u64 = 1_500;

const ENV_PORT: &str = "OPENRACING_WTCR_UDP_PORT";
const ENV_HEARTBEAT_TIMEOUT_MS: &str = "OPENRACING_WTCR_HEARTBEAT_TIMEOUT_MS";
...
/// Lateral G normalisation range for FFB scalar (touring cars typically reach ±3 G).
const FFB_LAT_G_MAX: f32 = 3.0;

/// WTCR adapter for Codemasters Mode 1 UDP telemetry.
#[derive(Clone)]
pub struct WtcrAdapter {
    bind_port: u16,
...
pub fn parse_wtcr_packet(data: &[u8]) -> Result<NormalizedTelemetry> {
    if data.len() < MIN_PACKET_SIZE {
        return Err(anyhow!(
            "WTCR packet too short: need at least {} bytes, got {}",
            MIN_PACKET_SIZE,
            data.len()
        ));
    }
...
#[async_trait]
impl TelemetryAdapter for WtcrAdapter {
    fn game_id(&self) -> &str {
        "wtcr"
    }
        ...
        tokio::spawn(async move {
            ...
            let socket = match TokioUdpSocket::bind(bind_addr).await {
                ...
                Err(error) => {
                    warn!(
                        error = %error,
                        port = bind_port,
                        "WTCR UDP socket bind failed"
                    );
                    return;
                }
            };

            info!(port = bind_port, "WTCR UDP adapter bound");
            ...
            loop {
                ...
                let len = match recv {
                    Ok(Ok(len)) => len,
                    Ok(Err(error)) => {
                        warn!(error = %error, "WTCR UDP receive error");
                        continue;
                    }
                    Err(_) => {
                        debug!("WTCR UDP receive timeout");
                        continue;
                    }
                };

                let data = &buf[..len];
                let normalized = match parse_wtcr_packet(data) {
                    Ok(n) => n,
                    Err(error) => {
                        warn!(error = %error, "Failed to parse WTCR packet");
                        continue;
                    }
                };
...
mod tests {
    ...
    #[test]
    fn zero_packet_returns_zero_speed_and_rpm() -> TestResult {
        let adapter = WtcrAdapter::new();
    ...
    #[test]
    fn speed_extracted_from_wheel_speeds() -> TestResult {
        let adapter = WtcrAdapter::new();
    ...
    #[test]
    fn ffb_scalar_clamped_to_range() -> TestResult {
        let adapter = WtcrAdapter::new();
        let mut raw = make_packet(MIN_PACKET_SIZE);
    ...
    #[test]
    fn game_id_is_wtcr() {
        assert_eq!(WtcrAdapter::new().game_id(), "wtcr");
    }
}
...
mod property_tests {
    ...
    proptest! {
        ...
        fn prop_short_packet_returns_err(len in 0usize..MIN_PACKET_SIZE) {
            ...
            prop_assert!(parse_wtcr_packet(&data).is_err());
        }
        ...
        fn prop_throttle_brake_clamped(
            ...
            if let Ok(t) = parse_wtcr_packet(&buf) {
        ...
        #[test]
        fn prop_ffb_scalar_in_range(lat_g in any::<f32>()) {
            let mut buf = vec![0u8; MIN_PACKET_SIZE];
            buf[OFF_GFORCE_LAT..OFF_GFORCE_LAT + 4].copy_from_slice(&lat_g.to_le_bytes());
            if let Ok(t) = parse_wtcr_packet(&buf) {
