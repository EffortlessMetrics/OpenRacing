//! Race Driver: GRID telemetry adapter for Codemasters Mode 1 UDP format.
//!
...
const ENV_PORT: &str = "OPENRACING_RACE_DRIVER_GRID_UDP_PORT";
const ENV_HEARTBEAT_TIMEOUT_MS: &str = "OPENRACING_RACE_DRIVER_GRID_HEARTBEAT_TIMEOUT_MS";

/// Race Driver: GRID adapter for Codemasters Mode 1 UDP telemetry.
#[derive(Clone)]
pub struct RaceDriverGridAdapter {
    bind_port: u16,
...
impl Default for RaceDriverGridAdapter {
    fn default() -> Self {
        Self::new()
    }
}

impl RaceDriverGridAdapter {
    pub fn new() -> Self {
...
fn parse_packet(data: &[u8]) -> Result<NormalizedTelemetry> {
    codemasters_shared::parse_codemasters_mode1_common(data, "Race Driver: GRID")
}

#[async_trait]
impl TelemetryAdapter for RaceDriverGridAdapter {
    fn game_id(&self) -> &str {
        "race_driver_grid"
    }
        ...
        tokio::spawn(async move {
            ...
            let socket = match TokioUdpSocket::bind(bind_addr).await {
                ...
                Err(error) => {
                    warn!(
                        error = %error,
                        port = bind_port,
                        "Race Driver: GRID UDP socket bind failed"
                    );
                    return;
                }
            };
            ...
            loop {
                ...
                let len = match recv {
                    Ok(Ok(len)) => len,
                    Ok(Err(error)) => {
                        warn!(error = %error, "Race Driver: GRID UDP receive error");
                        continue;
                    }
                    Err(_) => {
                        debug!("Race Driver: GRID UDP receive timeout");
                        continue;
                    }
                };
                ...
                let normalized = match parse_packet(data) {
                    Ok(n) => n,
                    Err(error) => {
                        warn!(error = %error, "Failed to parse Race Driver: GRID packet");
                        continue;
                    }
                };
