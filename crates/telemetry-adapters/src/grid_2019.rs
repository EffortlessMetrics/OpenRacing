//! GRID 2019 telemetry adapter for Codemasters Mode 1 UDP format.
//!
...
//! GRID Autosport, GRID Legends, and the broader Codemasters series.
...
const ENV_PORT: &str = "OPENRACING_GRID_2019_UDP_PORT";
const ENV_HEARTBEAT_TIMEOUT_MS: &str = "OPENRACING_GRID_2019_HEARTBEAT_TIMEOUT_MS";
...
/// GRID 2019 adapter for Codemasters Mode 1 UDP telemetry.
#[derive(Clone)]
pub struct Grid2019Adapter {
    bind_port: u16,
...
impl Default for Grid2019Adapter {
    fn default() -> Self {
        Self::new()
    }
}

impl Grid2019Adapter {
    pub fn new() -> Self {
...
fn parse_packet(data: &[u8]) -> Result<NormalizedTelemetry> {
    if data.len() < MIN_PACKET_SIZE {
        return Err(anyhow!(
            "GRID 2019 packet too short: need at least {} bytes, got {}",
            MIN_PACKET_SIZE,
            data.len()
        ));
    }
...
#[async_trait]
impl TelemetryAdapter for Grid2019Adapter {
    fn game_id(&self) -> &str {
        "grid_2019"
    }
        ...
        tokio::spawn(async move {
            ...
            let socket = match TokioUdpSocket::bind(bind_addr).await {
                ...
                Err(error) => {
                    warn!(
                        error = %error,
                        port = bind_port,
                        "GRID 2019 UDP socket bind failed"
                    );
                    return;
                }
            };

            info!(port = bind_port, "GRID 2019 UDP adapter bound");
            ...
            loop {
                ...
                let len = match recv {
                    Ok(Ok(len)) => len,
                    Ok(Err(error)) => {
                        warn!(error = %error, "GRID 2019 UDP receive error");
                        continue;
                    }
                    Err(_) => {
                        debug!("GRID 2019 UDP receive timeout");
                        continue;
                    }
                };
                ...
                let normalized = match parse_packet(data) {
                    Ok(n) => n,
                    Err(error) => {
                        warn!(error = %error, "Failed to parse GRID 2019 packet");
                        continue;
                    }
                };
...
mod tests {
    ...
    fn speed_extracted_from_wheel_speeds() -> Result<(), Box<dyn std::error::Error>> {
        ...
        write_f32(&mut raw, OFF_WHEEL_SPEED_FL, 50.0);
        write_f32(&mut raw, OFF_WHEEL_SPEED_FR, 50.0);
        write_f32(&mut raw, OFF_WHEEL_SPEED_RL, 50.0);
        write_f32(&mut raw, OFF_WHEEL_SPEED_RR, 50.0);
        let t = adapter.normalize(&raw)?;
        assert!((t.speed_ms - 50.0).abs() < 0.001, "speed_ms should be 50.0, got {}", t.speed_ms);
        Ok(())
    }

    #[test]
    fn game_id_is_grid_2019() {
