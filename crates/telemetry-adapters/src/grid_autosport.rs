...
//! WRC Generations, and the broader GRID series.
...
/// Lateral G normalisation range for FFB scalar (circuit cars typically reach ±3 G).
...
fn parse_packet(data: &[u8]) -> Result<NormalizedTelemetry> {
    if data.len() < MIN_PACKET_SIZE {
        return Err(anyhow!(
            "GRID Autosport packet too short: need at least {} bytes, got {}",
            MIN_PACKET_SIZE,
            data.len()
        ));
    }
...
mod tests {
    ...
    fn speed_extracted_from_wheel_speeds() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let t = adapter.normalize(&raw)?;
        assert!((t.speed_ms - 30.0).abs() < 0.001, "speed_ms should be 30.0, got {}", t.speed_ms);
        Ok(())
    }
