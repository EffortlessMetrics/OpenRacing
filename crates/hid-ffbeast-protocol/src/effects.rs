...
//! and device control.
//!
//! The report IDs follow USB HID PID 1.01 and are compatible with DirectInput
//! on Windows and the Linux kernel `hid-pidff` driver.
//!
//! # Sources
//!
...
///
/// Note: constant force (0x01) is in `output.rs`. FFB enable (0x60) and
/// gain (0x61) are vendor feature reports also in `output.rs`.
pub mod report_ids {
    /// Set Effect report.
    ...
    /// Device Gain report (standard PID; separate from vendor 0x61).
...
///
/// This is the PIDFF Set Constant Force parameter report. For the
/// real-time torque output command, see [`crate::output::FFBeastTorqueEncoder`].
...
/// Encode a standard PIDFF Device Gain report (4 bytes, gain 0-10000).
///
/// This is the standard PIDFF device gain report. FFBeast also has a
/// vendor-specific gain feature report at 0x61 (see [`crate::output::build_set_gain`]).
...
mod tests {
    ...
    fn set_effect_report_layout() {
        ...
        assert_eq!(buf[9], 255);
        assert_eq!(buf[10], 0xFF); // no trigger
