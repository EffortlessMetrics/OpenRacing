...
proptest! {
    ...
    /// is_ffbeast_product must only recognise the three known PIDs.
    #[test]
    ...
    #[test]
    fn prop_deterministic_encoding(torque in -1.0f32..1.0f32) {
        let enc = FFBeastTorqueEncoder;
        let r1 = enc.encode(torque);
        let r2 = enc.encode(torque);
        prop_assert_eq!(r1, r2, "encoding must be deterministic");
    }
}

/// Zero torque must encode to zero raw value.
#[test]
fn test_zero_torque_encodes_to_zero_raw() -> Result<(), String> {
    let enc = FFBeastTorqueEncoder;
    let report = enc.encode(0.0);
    let raw = i16::from_le_bytes([report[1], report[2]]);
    if raw != 0 {
        return Err(format!("expected raw=0 for torque=0.0, got {raw}"));
    }
    Ok(())
}
