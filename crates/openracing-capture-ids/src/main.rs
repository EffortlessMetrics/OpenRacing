...
mod tests {
    use super::*;
    use crate::replay::{decode_hex, parse_capture_line, parse_vid_str, CapturedReport};
    ...
    #[test]
    fn given_moza_vid_when_valid_report_decoded_then_identified_as_moza_with_axes() -> anyhow::Result<()> {
        ...
        assert!(text.starts_with("MOZA:"), "Expected MOZA prefix, got: {text}");
        assert!(text.contains("steering="), "Missing steering field in: {text}");
        assert!(text.contains("throttle="), "Missing throttle field in: {text}");
    ...
    #[test]
    fn given_logitech_vid_when_valid_report_decoded_then_identified_as_logitech_with_axes() -> anyhow::Result<()> {
        ...
        assert!(text.starts_with("Logitech:"), "Expected Logitech prefix, got: {text}");
        assert!(text.contains("steering="), "Missing steering field in: {text}");
        assert!(text.contains("throttle="), "Missing throttle field in: {text}");
        assert!(text.contains("brake="), "Missing brake field in: {text}");
        assert!(text.contains("buttons="), "Missing buttons field in: {text}");
        Ok(())
    }
    ...
    #[test]
    fn given_moza_full_deflection_when_decoded_then_values_normalised_correctly() -> anyhow::Result<()> {
        ...
        assert!(text.contains("steering=1.000"), "Expected full steering, got: {text}");
        assert!(text.contains("throttle=1.000"), "Expected full throttle, got: {text}");
        assert!(text.contains("brake=0.000"), "Expected zero brake, got: {text}");
        Ok(())
    }
    ...
    fn given_logitech_full_left_and_throttle_when_decoded_then_values_correct() -> anyhow::Result<()> {
        ...
        assert!(text.contains("steering=-1.000"), "Expected full-left steering, got: {text}");
        assert!(text.contains("throttle=1.000"), "Expected full throttle, got: {text}");
        assert!(text.contains("brake=0.000"), "Expected zero brake, got: {text}");
        Ok(())
    }
    ...
    #[test]
    fn given_moza_capture_line_when_parsed_and_decoded_then_pipeline_produces_output() -> anyhow::Result<()> {
    ...
    #[test]
    fn given_logitech_capture_line_when_full_pipeline_executed_then_output_formatted() -> anyhow::Result<()> {
        // Logitech capture: id=0x01, steering=center, all zero, hat=neutral
        let line = r#"{"ts_ns":2000000000,"vid":"0x046D","pid":"0x0001","report":"01008000000000000800"}"#;
        ...
        assert!(decoded.contains("steering=0.000"), "Center steering should be 0.000, got: {decoded}");
        Ok(())
    }
    ...
    #[test]
    fn given_sequential_captures_when_parsed_then_timestamps_monotonically_increasing() -> anyhow::Result<()> {
        let lines = [
        ...
        for line in &lines {
            let entry = parse_capture_line(line)?;
            assert!(entry.ts_ns > prev_ts, "Timestamps must increase: {} <= {}", entry.ts_ns, prev_ts);
            prev_ts = entry.ts_ns;
        }
        Ok(())
    }
    ...
    fn given_unknown_vid_when_decode_attempted_then_returns_none() {
        ...
        assert!(decode_report(0xFFFF, &report).is_none(), "Unknown VID 0xFFFF should return None");
        assert!(decode_report(0x0000, &report).is_none(), "Zero VID should return None");
        assert!(decode_report(0x1234, &report).is_none(), "Arbitrary VID should return None");
    }
    ...
    fn given_known_vid_with_malformed_report_when_decoded_then_returns_none_gracefully() {
        ...
        assert!(decode_report(0x346E, &short_report).is_none(), "Short MOZA report should return None");

        // Logitech VID with too-short report (< 10 bytes minimum)
        let short_report: [u8; 4] = [0x01, 0x00, 0x80, 0x00];
        assert!(decode_report(0x046D, &short_report).is_none(), "Short Logitech report should return None");
    }
    ...
    fn given_known_vid_with_wrong_report_id_when_decoded_then_returns_none() {
        ...
        assert!(decode_report(0x346E, &report).is_none(), "Wrong report ID for MOZA should return None");

        // Logitech report with wrong report ID
        let report: [u8; 10] = [0x02, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00];
        assert!(decode_report(0x046D, &report).is_none(), "Wrong report ID for Logitech should return None");
    }
    ...
    #[test]
    fn given_multiple_devices_when_decoded_then_separate_protocol_handlers_used() -> anyhow::Result<()> {
        ...
        assert!(moza_text.starts_with("MOZA:"), "Device A should use MOZA handler");
        assert!(logi_text.starts_with("Logitech:"), "Device B should use Logitech handler");
        // Then: no cross-contamination between device outputs
        assert!(!moza_text.contains("Logitech"), "MOZA output must not contain Logitech data");
        assert!(!logi_text.contains("MOZA"), "Logitech output must not contain MOZA data");
        Ok(())
    }
    ...
    fn given_interleaved_multi_device_captures_when_decoded_then_no_conflicts() -> anyhow::Result<()> {
        ...
        for output in &moza_outputs {
            assert!(output.starts_with("MOZA:"), "All MOZA outputs should use MOZA handler");
        }
        for output in &logi_outputs {
            assert!(output.starts_with("Logitech:"), "All Logitech outputs should use Logitech handler");
        }
        Ok(())
    }
