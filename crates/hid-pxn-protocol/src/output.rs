//! PXN FFB output report encoding.
//!
...
/// Report ID for PXN FFB output reports.
///
/// PXN uses report ID `0x05` for constant-force output, unlike the standard `0x01`
/// used by some other PIDFF devices.
pub const FFB_REPORT_ID: u8 = 0x05;

/// Encode a normalised torque command as a PXN FFB output report.
///
...
pub fn encode_torque(torque_normalized: f32) -> [u8; FFB_REPORT_LEN] {
    ...
    [FFB_REPORT_ID, bytes[0], bytes[1], 0x00, 0x00, 0x00, 0x00, 0x00]
}
...
mod tests {
    ...
    fn encode_zero() {
        ...
        assert_eq!(&report[3..], &[0x00, 0x00, 0x00, 0x00, 0x00]);
    }
