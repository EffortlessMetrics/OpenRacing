//! High-level verification interface for OpenRacing

#![deny(clippy::unwrap_used)]

use crate::ed25519::Ed25519Verifier;
use crate::trust_store::TrustStore;
use crate::{CryptoError, SignatureMetadata, SignatureVerifier, TrustLevel};
...
impl Default for VerificationConfig {
    fn default() -> Self {
        Self {
            ...
            trust_store_path: PathBuf::from("trust_store"),
            max_signature_age_seconds: Some(365 * 24 * 3600),
        }
    }
}

/// Main verification service for OpenRacing
...
impl VerificationService {
    ...
    /// Create a verification service with an existing trust store
    pub fn with_trust_store(config: VerificationConfig, trust_store: TrustStore) -> Self {
        let verifier = Ed25519Verifier::new(trust_store);
...
pub mod verification_utils {
    ...
    pub fn detect_content_type(path: &Path) -> Option<ContentType> {
        ...
        match extension.as_deref() {
            ...
            Some("bin") => Some(ContentType::Binary),
            ...
            Some("wup") | Some("update") => Some(ContentType::Update),
            _ => None,
        }
    }
...
pub struct VerificationReport {
    ...
    /// Files that failed verification with error messages
...
mod tests {
    ...
    fn test_content_type_detection() -> Result<(), Box<dyn std::error::Error>> {
        let result = verification_utils::detect_content_type(Path::new("wheeld.exe"));
        assert!(matches!(result, Some(ContentType::Binary)));

        let result = verification_utils::detect_content_type(Path::new("firmware.fw"));
        assert!(matches!(result, Some(ContentType::Firmware)));

        let result = verification_utils::detect_content_type(Path::new("plugin.wasm"));
        assert!(matches!(result, Some(ContentType::Plugin)));

        let result = verification_utils::detect_content_type(Path::new("car.profile.json"));
        assert!(matches!(result, Some(ContentType::Profile)));

        let result = verification_utils::detect_content_type(Path::new("v1.0.0.wup"));
    ...
    fn test_should_verify_file() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(verification_utils::should_verify_file(
            Path::new("test.exe"),
            &ContentType::Binary,
            &config
        ));
