...
impl WasmPlugin {
    ...
    fn add_host_functions(linker: &mut Linker<WasmPluginState>) -> PluginResult<()> {
        ...
        linker.func_wrap(
            "env",
            "check_capability",
            |mut caller: Caller<'_, WasmPluginState>, capability_ptr: i32, capability_len: i32| -> i32 {
                let memory = match caller.get_export("memory") {
                    Some(Extern::Memory(mem)) => mem,
                    _ => return -1, // Error: no memory export
                };
                
                let data = match memory.data(&caller).get(capability_ptr as usize..(capability_ptr + capability_len) as usize) {
                    Some(data) => data,
                ...
                match capability_str {
                    "read_telemetry" => {
                        if caller.data().capability_checker.check_telemetry_read().is_ok() {
                            1 // Success
                        } else {
                            0 // Permission denied
                        }
                    ...
                    "modify_telemetry" => {
                        if caller.data().capability_checker.check_telemetry_modify().is_ok() {
                            1
                        } else {
                            0
                        }
        ...
        linker.func_wrap(
            ...
            |mut caller: Caller<'_, WasmPluginState>, level: i32, msg_ptr: i32, msg_len: i32| {
                ...
                let data = match memory.data(&caller).get(msg_ptr as usize..(msg_ptr + msg_len) as usize) {
                    Some(data) => data,
                    None => return,
                };
    ...
    async fn execute_with_limits(
        ...
        match result {
            Ok(Ok(_)) => {
                ...
                if execution_time.as_micros() > self.manifest.constraints.max_execution_time_us as u128 {
    ...
    async fn process_telemetry(
        ...
        if output_bytes.is_empty() {
            ...
            let modified_telemetry: NormalizedTelemetry = serde_json::from_slice(&output_bytes)
                .map_err(|e| PluginError::LoadingFailed(format!("Output deserialization: {}", e)))?;
    ...
    pub async fn execute_plugin(
        ...
        match operation {
            PluginOperation::TelemetryProcessor => {
                let telemetry: NormalizedTelemetry = serde_json::from_value(input_data)
                    .map_err(|e| PluginError::LoadingFailed(format!("Invalid telemetry data: {}", e)))?;
