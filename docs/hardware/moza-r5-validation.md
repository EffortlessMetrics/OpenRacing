...
These templates also default to `success: false`. The visible-motion follow-up template is only a review artifact after a response-only real visible-motion receipt; it does not authorize another output attempt, replace the preserved response/visible-motion receipt, or satisfy the visible-motion gate. The simulator templates must not be used as evidence until a real telemetry recording or bounded FFB smoke run fills the fields.
