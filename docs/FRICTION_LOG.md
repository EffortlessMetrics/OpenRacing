...
**Update (CI check):** `.github/workflows/yaml-sync-check.yml` + `cargo run -p wheelctl --bin yaml-sync-check -- --check` confirmed present. The workflow runs on every push/PR and fails with a clear diff message if the files diverge.

**Update (sync script):** `cargo run -p wheelctl --bin yaml-sync-check --` added — run `cargo run -p wheelctl --bin yaml-sync-check -- --fix` after editing the canonical file to keep both files in sync. See F-013 (Resolved). The single-source-of-truth refactor remains a long-term goal.
...
The CI check (`yaml-sync-check`) would catch divergence at PR time, but offered no fix path — developers had to manually copy the file or hunt down the right diff. This caused repeated F-001/F-013 CI failures.

**Fix applied:** `cargo run -p wheelctl --bin yaml-sync-check --` added. Run `cargo run -p wheelctl --bin yaml-sync-check -- --fix` after editing `crates/telemetry-config/src/game_support_matrix.yaml` to copy it to the mirror. Use `--check` in CI or pre-commit hooks to verify without writing. Documented in `docs/DEVELOPMENT.md` under "Keeping game support matrix files in sync". Implemented in Rust as part of the `wheelctl` crate.
...
**Fix applied:** `cargo run -p wheelctl --bin yaml-sync-check --` added — run `cargo run -p wheelctl --bin yaml-sync-check -- --fix` after editing the canonical file to copy it to the mirror. The long-term single-source-of-truth refactor remains tracked under F-001. (feat/r5-test-coverage-and-integration)
