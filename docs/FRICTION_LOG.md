...
### F-016 · `bench_results.json` generation undocumented (Low · Open)
...
**Remedy:** Add the generation command to `CLAUDE.md` under "Benchmarks and performance", and add a CI job that generates the file then immediately validates it via `validate_performance.py --strict`.
